...
<img src="rejoice.jpg" alt="thank_god" style="height: 200px; max-width: 48%">
...
One of the problems I ran into when testing my [elbow configuring](https://kem406.github.io/PHYS-S12/09/) was that the solenoid valve I used was not strong enough to withhold all of the air pressure coming from the tank when it was in its "closed" position. As a result, a little bit of air leaked through when no buttons were pressed, causing the arm to slowly rise without a given command to rise.
...
const int valveOne = 1;
const int valveTwo = 2;
const int valveThree =  3;
const int valveFour = 4;
const int valveFive = 5;
